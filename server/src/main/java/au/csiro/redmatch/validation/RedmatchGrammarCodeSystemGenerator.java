...
/**
 * Creates a Lucene index from the FHIR metadata.
 * 
...
public class RedmatchGrammarCodeSystemGenerator {
  ...
  private Map<String, StructureDefinition> resourceMap = new HashMap<>();
  private Map<String, StructureDefinition> primitiveTypeMap = new HashMap<>();
  private Map<String, StructureDefinition> complexTypeMap = new HashMap<>();
  ...
  private Set<String> allParents = new HashSet<>();
  ...
  private boolean isValueX(ElementDefinition ed) {
    if (ed.hasType() && ed.getType().size() > 1) {
      return true;
    } else {
      return false;
    }
  ...
  private void processElementDefinition(boolean nested, String prefix, final Deque<String> parents,
    ...
    if (!path.isEmpty()) {
      while (!path.startsWith(currentParent)) {
        parents.pop();
    ...
    if (trc == null) {
      ...
      if (ed.getType().size() > 1) {
        throw new RuntimeException("Found non-x-value with more than one type: " + ed.toString());
      }
    }
  ...
  public static void main (String[] args) {
    ...
    File output = new File("src/main/docker/ontoserver/preload.json");
    
    if (!output.exists()) {
      FhirVersionEnum fhirVersion = FhirVersionEnum.R4;
      RedmatchGrammarCodeSystemGenerator fi = new RedmatchGrammarCodeSystemGenerator(fhirVersion);
      try {
        output.getParentFile().mkdirs();
        output.createNewFile();
        
        final String base = "fhir-metadata/" + fhirVersion.getFhirVersionString();
        log.info("Loading FHIR metadata from " + base);
        File profileTypes = getFile(base + "/profiles-types.json");
        File profileResources = getFile(base + "/profiles-resources.json");

        FhirContext ctx = new FhirContext(fhirVersion);
        JsonParser parser = (JsonParser) ctx.newJsonParser().setPrettyPrint(true);
        final Bundle types = parser.doParseResource(Bundle.class, new FileReader(profileTypes));
        ...
        e.printStackTrace();
        System.err.println("A file was not found: " + e.getLocalizedMessage());
        System.exit(-1);
      } catch (IOException e) {
        e.printStackTrace();
        System.err.println("There was a problem creating the code system: " 
            + e.getLocalizedMessage());
        System.exit(-1);
      }
    } else {
      System.out.println("Code system file already exists.");
    }
  }
  
  class ConceptDefinitionComponentElementDefinitionPair {
